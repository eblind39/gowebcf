...
func (app *Config) PostRegisterPage(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("http://localhost:3007/activate?email=%s", u.Email)
...
func (app *Config) ActivateAccount(w http.ResponseWriter, r *http.Request) {
	// validate url
	url := r.RequestURI
	testURL := fmt.Sprintf("http://localhost:3007%s", url)
...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		err := pdf.OutputFileAndClose(fmt.Sprintf("./go-concurrency/finalproj/tmp/%d_manual.pdf", user.ID))
		if err != nil {
			app.ErrorChan <- err
			return
		}
		...
		msg := Message{
			...
			AttachmentsMap: map[string]string{
				"Manual.pdf": fmt.Sprintf("./go-concurrency/finalproj/tmp/%d_manual.pdf", user.ID),
			},
		}
...
func (app *Config) generateManual(u data.User, plan *data.Plan) *gofpdf.Fpdf {
	...
	t := importer.ImportPage(pdf, "./go-concurrency/finalproj/pdf/manual.pdf", 1, "/MediaBox")
	pdf.AddPage()
